...
function pbkdf2_bash {
  ...
  # */
  local -ai key salt dk u t block1
  local -i iterations=$3 dkLen=$4 hLen=64 i j k destPos len
  ...
  do
    ...
    u=($(
      ...
      openssl dgst -sha512 -hmac "$1" -binary |
      xxd -p -c 1 |
      while read
      do echo $((0x$REPLY))
      done
...
function bip39() {
  ...
  then 
    ...
    pbkdf2_python sha512 "$*" "mnemonic$passphrase" 2048
  elif (($# == 0))
  then $FUNCNAME 160
  else
