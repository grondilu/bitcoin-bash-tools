  ...
  then cat <<-ENDUSAGE
	...
	then echo "private key derivation can't be done from a public key" >&2; return 2
      ...
      mnemonic-new)
        ...
        then 
          local -a wordlist=($(cat english.txt))
	  local -i ent=${#1}*4 #bits
	  if ((ent % 32))
	  then
	    1>&2 echo The seed length in bytes is not evenly divisible by 32 bits.
	    return 2
          elif ((ent > 256))
          then
            1>&2 echo "The algorithm for long seeds (256+ bits) is not correctly implemented yet"
            return 3
          else
	    { 
